...
static CBlock CreateGenesisBlock(const char* pszTimestamp, const CScript& genesisOutputScript, const uint32_t nTime, const uint32_t nNonce, const uint32_t nBits, const int32_t nVersion, const CAmount& genesisReward)
{
    ...
    txNew.vin.resize(1);
    txNew.vout.resize(1);
    txNew.vin[0].scriptSig = CScript() << 1480527777 << CScriptNum(4) << std::vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
...
public:
    CMainParams() {
        ...
        consensus.nDynodePaymentsStartBlock = 100; // TODO: SWITCH BACK: Dynode payments start at block 10,000
        ...
        consensus.nPowTargetTimespan = 10 * 64; // Dynamic: 12 hours
        consensus.nPowTargetSpacing = 1 * 64; // Dynamic: 64 seconds
        ...
        genesis = CreateGenesisBlock(1480527777 , 3943, UintToArith256(consensus.powLimit).GetCompact(), 1, (1 * COIN));
        if(startNewChain == true) { MineGenesis(genesis, consensus.powLimit); }

        consensus.hashGenesisBlock = genesis.GetHash();
        assert(consensus.hashGenesisBlock == uint256S("0x0000e45e306478b104ed272760042ebf35f92ec6bd6d8358b18a8b1d4ffb3add"));
        assert(genesis.hashMerkleRoot == uint256S("0x33cf91aad179bc4aedf4accf2a523691050a85967985eac3d8a59313900bea47"));
        ...
        checkpointData = (CCheckpointData) {
            ...
            (  0, uint256S("0x0000e45e306478b104ed272760042ebf35f92ec6bd6d8358b18a8b1d4ffb3add")),
            1480527777 , // * UNIX timestamp of last checkpoint block
...
public:
    CTestNetParams() {
        strNetworkID = "test";
        consensus.nWorkSubsidyStartHeight = 10000; // Proof-Of-Work Reward after height
        consensus.nDynodePaymentsStartBlock = 200; // not true, but it's ok as long as it's less then nDynodePaymentsIncreaseBlock
        consensus.nInstantSendKeepLock = 24;
        consensus.nInstantSendReprocessBlocks = 15;
...
public:
    CRegTestParams() {
        ...
        consensus.nWorkSubsidyStartHeight = 10000; // Proof-Of-Work Reward after height
        consensus.nDynodePaymentsStartBlock = 240;
        consensus.nInstantSendKeepLock = 24;
        consensus.nInstantSendReprocessBlocks = 15;
